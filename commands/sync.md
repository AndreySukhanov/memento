...
0. **Correction lenses + current phase (Rule 6)** - if this session invalidated
   earlier entries (wrong attribution, wrong place, wrong assumption), do NOT edit
   them: add a lens block at the top of `MEMORY.md` so all older text is read
   through it. Refresh the `## Current phase` block if the situation flipped.
...
6. **Session auto-memory (Rule 8)** - one fact = one file with frontmatter
   (`name`, `description`, `type`: feedback/project/reference/user), plus a
   one-line row in the auto-memory `MEMORY.md` index. Update the task's
   `project_*` note (status + date); if the user stated a new standing
   preference this session, write a `feedback_*` file for it.
